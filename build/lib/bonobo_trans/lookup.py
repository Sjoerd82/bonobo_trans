...
class DbLookup(Configurable):
	...
	provided via a Bonobo Service, containing a 'sqlalchemy.engine'.
	...
	Configuration options:
		...
		Optional (selection):
		-	table_name        (str) only optional if sql_override provided
	...
	Notes on options:
		...
		Not all options can be used together, generally, the lookup	trans-
		formation will issue a warning or error, depending on the severity,
		when invalid combinations are used.
		
		
	Notes on "source-data" selection:
	...
	-	table_name, sql_override
		...
		ignored. The 'sql_override' can either be a SQL statement string or a
	...
	-	order_by
		...
		Don't specify more than one column in the dictionary, as Python does
		not	guarantee the order of dict keys.
		
	
	Notes on comparison and conditions:
	...
	-	Special key: _compare_with_
		...
		The condition value can be of three different types:
			...
			LKP_VAL_COL		The 'value' is actually a column name available in
							the d_row (default)
			LKP_VAL_VAL		The 'value' is hardcoded (default fallback, if
							'value' is not a valid column)
	...
	-	SQLAlchemy resultset
		Whenever SQLite or Pandas are not available.
	...
	-	verbose_init
			If 'verbose_init' is True, all interesting details are printed to
			the output.
			
	-	verbose_sql
			If 'verbose_sql' is True, the generated SQL statement for the 
			source data and lookups will be printed.
