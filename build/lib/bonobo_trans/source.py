...
class DbSource(Configurable):
	...
	Configuration options:
		...
		Optional:
		-	sql_select		(str or sqlalchemy.sql)		Selection query
		-	sql_pre			(str or sqlalchemy.sql)		Pre-SQL
		-	sql_filter		(str or sqlalchemy.where)
		-	bp_select		(dict)						Bound parameter(s)
		...
		-	row_counters 	(bool)			True
		-	keep_alive	 	(bool)			False

	Notes on selection options:
	...
	-	sql_filter
		...
		It is not possible to specify a sql_filter when the sql_select contains
		an ORDER BY clause!
		
	-	ordered_cols

		An ORDER BY should ideally be specified via the ordered_cols parameter:
	...
	Notes on bound parameter options:
