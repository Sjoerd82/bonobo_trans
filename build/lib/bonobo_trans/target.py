...
class DbTarget(Configurable):
	...
	Configuration options:
		...
		-	bp_update		(dict)							Bound parameter(s)
		-	bp_pre_sql		(dict)							Bound parameter(s)		
	...
	Notes on operation and upsert_strategy:
	
		Operation can hold any of the following values:
		...
		If operation is TGT_OPER_UPSERT, the following additional options are
		available, specifiying how to execute the upsert.
	...
	Notes on bound pre sql bind parameter:

		Bound parameters must be provided using a dictionary.

		For 'raw' SQL, bound parameters are specified by name.
		For example:

				sql_pre = 'DELETE * FROM users WHERE id=:user_id'
				bp_sql_pre = { 'user_id':555 }
	...
	Notes on optional options with default:
